...
impl DynamicBsnLoader {
    /// Scan the parsed AST for named entries with asset types (e.g., StandardMaterial)
    /// and register them as labeled sub-assets so they're resolvable via
    /// `AssetServer::load("file.bsn#name")`.
    fn register_labeled_assets(
        ...
        // Check if top-level is a Children wrapper — unwrap to get entries
        ...
        for entry_id in entries {
            ...
            // Create a default asset, apply BSN fields
            ...
            // Convert to ErasedLoadedAsset and register as labeled sub-asset
        ...
        match *expr {
            ...
            BsnExpr::StringLit(ref string) => {
                    ...
                    .starts_with("bevy_asset::handle::HandleTemplate<")
                {
                    ...
                    let ReflectMut::Struct(reflect_struct) = reflect.reflect_mut() else {
                        panic!("`HandleTemplate` wasn't a struct")
                    };
                    reflect_struct.field_mut("path").unwrap().apply(&asset_path);
                    return Ok(reflect.into_partial_reflect());
                }
